...
func addNonce(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		db := stepacme.MustDatabaseFromContext(r.Context())
...
func addDirLink(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Add("Link", link(linker.GetLink(ctx, stepacme.DirectoryLinkType), "index"))
		next(w, r)
	}
}
...
func verifyContentType(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		u := &url.URL{
			Path: stepacme.GetUnescapedPathSuffix(stepacme.CertificateLinkType, p.GetName(), ""),
		}
		...
		render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType,
...
func parseJWS(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			render.Error(w, r, stepacme.WrapErrorISE(err, "failed to read request body"))
			return
		}
		jws, err := jose.ParseJWS(string(body))
		if err != nil {
			render.Error(w, r, stepacme.WrapError(stepacme.ErrorMalformedType, err, "failed to parse JWS from request body"))
			return
		}
		ctx := context.WithValue(r.Context(), contextKeyJWS, jws)
...
func validateJWS(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(jws.Signatures) == 0 {
			render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType, "request body does not contain a signature"))
			return
		}
		if len(jws.Signatures) > 1 {
			render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType, "request body contains more than one signature"))
			return
		}
		...
		if uh.KeyID != "" ||
			...
			render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType, "unprotected header must not be used"))
			return
		}
		hdr := sig.Protected
		...
		case jose.ES256, jose.ES384, jose.ES512, jose.EdDSA:
			// we good
		default:
			render.Error(w, r, stepacme.NewError(stepacme.ErrorBadSignatureAlgorithmType, "unsuitable algorithm: %s", hdr.Algorithm))
			return
		}

		// Check the validity/freshness of the Nonce.
		if err := db.DeleteNonce(ctx, stepacme.Nonce(hdr.Nonce)); err != nil {
			render.Error(w, r, err)
			return
		}
		...
		if !ok {
			render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType, "jws missing url protected header"))
			return
		}
		...
		if hdr.JSONWebKey != nil && hdr.KeyID != "" {
			render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType, "jwk and kid are mutually exclusive"))
			return
		}
		if hdr.JSONWebKey == nil && hdr.KeyID == "" {
			render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType, "either jwk or kid must be defined in jws protected header"))
			return
		}
		next(w, r)
	}
...
func extractJWK(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if jwk == nil {
			render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType, "jwk expected in protected header"))
			return
		}
		if !jwk.Valid() {
			render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType, "invalid jwk in protected header"))
			return
		}

		// Overwrite KeyID with the JWK thumbprint.
		jwk.KeyID, err = stepacme.KeyToID(jwk)
		if err != nil {
			render.Error(w, r, stepacme.WrapErrorISE(err, "error getting KeyID from JWK"))
			return
		}

		// Store the JWK in the context.
		ctx = context.WithValue(ctx, contextKeyJWK, jwk)
		...
		switch {
		case stepacme.IsErrNotFound(err):
		...
		default:
			if !acc.IsValid() {
				render.Error(w, r, stepacme.NewError(stepacme.ErrorUnauthorizedType, "account is not active"))
				return
			}
			ctx = context.WithValue(ctx, contextKeyAccount, acc)
		}
		next(w, r.WithContext(ctx))
	}
}
...
func checkPrerequisites(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		checkFunc, ok := stepacme.PrerequisitesCheckerFromContext(ctx)
		if ok {
			ok, err := checkFunc(ctx)
			if err != nil {
				render.Error(w, r, stepacme.WrapErrorISE(err, "error checking acme provisioner prerequisites"))
				return
			}
			if !ok {
				render.Error(w, r, stepacme.NewError(stepacme.ErrorNotImplementedType, "acme provisioner configuration lacks prerequisites"))
				return
			}
		}
		next(w, r)
...
func lookupJWK(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if kid == "" {
			render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType, "signature missing 'kid'"))
			return
		}
		...
		switch {
		case stepacme.IsErrNotFound(err):
			render.Error(w, r, stepacme.NewError(stepacme.ErrorAccountDoesNotExistType, "account with ID '%s' not found", accID))
			return
		case err != nil:
			render.Error(w, r, err)
			return
		default:
			...
			if storedLocation := acc.GetLocation(); storedLocation != "" {
				...
				reqProv := stepacme.MustProvisionerFromContext(ctx)
				switch {
				...
				linker := stepacme.MustLinkerFromContext(ctx)
				kidPrefix := linker.GetLink(ctx, stepacme.AccountLinkType, "")
			...
			ctx = context.WithValue(ctx, contextKeyAccount, acc)
			ctx = context.WithValue(ctx, contextKeyJWK, acc.Key)
			next(w, r.WithContext(ctx))
			return
		}
	}
...
func verifyAndExtractJWSPayload(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if jwk.Algorithm != "" && jwk.Algorithm != jws.Signatures[0].Protected.Algorithm {
			render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType, "verifier and signature algorithm do not match"))
			return
		}
		...
		case errors.Is(err, jose.ErrCryptoFailure):
			...
			if err != nil {
				render.Error(w, r, stepacme.WrapError(stepacme.ErrorMalformedType, err, "error verifying jws with patched signature(s)"))
				return
			}
		case err != nil:
			render.Error(w, r, stepacme.WrapError(stepacme.ErrorMalformedType, err, "error verifying jws"))
			return
		}

		ctx = context.WithValue(ctx, contextKeyPayload, &payloadInfo{
			value:       payload,
...
func isPostAsGet(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !payload.isPostAsGet {
			render.Error(w, r, stepacme.NewError(stepacme.ErrorMalformedType, "expected POST-as-GET"))
			return
		}
		next(w, r)
	}
...
// account or an error.
func accountFromContext(ctx context.Context) (*stepacme.Account, error) {
	val, ok := ctx.Value(contextKeyAccount).(*stepacme.Account)
	if !ok || val == nil {
		return nil, stepacme.NewError(stepacme.ErrorAccountDoesNotExistType, "account not in context")
	}
	return val, nil
}
...
func jwkFromContext(ctx context.Context) (*jose.JSONWebKey, error) {
	val, ok := ctx.Value(contextKeyJWK).(*jose.JSONWebKey)
	if !ok || val == nil {
		return nil, stepacme.NewErrorISE("jwk expected in request context")
	}
	return val, nil
}
...
func jwsFromContext(ctx context.Context) (*jose.JSONWebSignature, error) {
	val, ok := ctx.Value(contextKeyJWS).(*jose.JSONWebSignature)
	if !ok || val == nil {
		return nil, stepacme.NewErrorISE("jws expected in request context")
	}
	return val, nil
}
...
func provisionerFromContext(ctx context.Context) (stepacme.Provisioner, error) {
	p, ok := stepacme.ProvisionerFromContext(ctx)
	if !ok || p == nil {
		return nil, stepacme.NewErrorISE("provisioner expected in request context")
	}
	return p, nil
}
...
func acmeProvisionerFromContext(ctx context.Context) (*provisioner.ACME, error) {
	...
	if !ok {
		return nil, stepacme.NewErrorISE("provisioner in context is not an ACME provisioner")
	}
...
func payloadFromContext(ctx context.Context) (*payloadInfo, error) {
	val, ok := ctx.Value(contextKeyPayload).(*payloadInfo)
	if !ok || val == nil {
		return nil, stepacme.NewErrorISE("payload expected in request context")
	}
	return val, nil
}
