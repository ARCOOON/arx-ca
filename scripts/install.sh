...
ExecStartPre=+/usr/bin/chown -R ${SYSTEM_USER}:${SYSTEM_USER} ${APP_DIR}
ExecStartPre=+/usr/bin/chmod 600 ${APP_DIR}/server.yaml
ExecStartPre=+/usr/bin/chmod 700 ${APP_DIR}/arx
