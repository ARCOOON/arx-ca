...
export async function fetchSettingsConfig(): Promise<SettingsConfigResponse> {
  ...
  if (!payload.data) throw new Error('Settings config response did not include data')
  return payload.data
}
...
export async function updateSettingsConfig(
  ...
  const payload = response.data
  if (payload.error) throw new Error(payload.error)
  if (!payload.data) throw new Error('Settings config update response did not include data')
  return payload.data
}
